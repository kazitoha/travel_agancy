...
@section('admin-content')
    <div class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h1 class="text-xl font-bold text-slate-900">References</h1>
            <p class="mt-1 text-sm text-slate-500">Create and manage your reference list.</p>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-1">
                <div class="text-sm font-bold text-slate-900">Add reference</div>

                <form class="mt-5 space-y-4" method="POST" action="{{ route('references.store') }}">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="3"
                            class="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none ring-blue-200 focus:border-blue-300 focus:ring-4">{{ old('address') }}</textarea>
                    </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
                <div class="text-sm font-bold text-slate-900">Reference list</div>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse ($references as $reference)
                                    <tr class="bg-white hover:bg-slate-50">
                                        <td class="px-4 py-3 font-semibold text-slate-900">{{ $reference->company_name }}</td>
                                        <td class="px-4 py-3 text-slate-700">{{ $reference->contact_person_name }}</td>
                                        <td class="px-4 py-3 text-slate-700">{{ $reference->phone }}</td>
                                        <td class="px-4 py-3 text-slate-700">{{ $reference->address }}</td>
                                        <td class="px-4 py-3">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-500">No references found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
